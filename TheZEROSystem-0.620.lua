...
recenthit[2] = 0
combop = {}
combop[1] = mainmemory.read_u8(0x1BA8)
combop[2] = mainmemory.read_u8(0x1BA8)
firstframeofhitp = {}
firstframeofhitp[1] = 0
firstframeofhitp[2] = 0
health = {}
health[1] = mainmemory.read_s16_le(0x1B70)
health[2] = mainmemory.read_s16_le(0x1B74)
totaldamage = {}
totaldamage[1] = 0
totaldamage[2] = 0
stageselectlabels = {}
stageselectlabels[0] = "Default"
stageselectlabels[1] = "Heavyarms"
stageselectlabels[2] = "Deathscythe"
stageselectlabels[3] = "Shenlong"
stageselectlabels[4] = "Wing/Wing.0"
stageselectlabels[5] = "Sandrock"
stageselectlabels[6] = "Vayaete"
stageselectlabels[7] = "Tallgeese"
stageselectlabels[8] = "Epyon"
stageselectlabels[9] = "Mercurius"
stageselectlabels[10] = "???"
stageselectlabels[11] = "????"
...
function globals()
	...
	firstframeofhitp[1] = ((firstframeofhitp[1] + 1) * ((mainmemory.read_u8(0x1BA8)>0 and mainmemory.read_u8(0x1BA8)>combop[1]) and 1 or 0))-- + (mainmemory.read_u8(0x1BA8)>combop[1] and 0 or 0)
	firstframeofhitp[2] = ((firstframeofhitp[2] + 1) * ((mainmemory.read_u8(0x1BAC)>0 and mainmemory.read_u8(0x1BAC)>combop[2]) and 1 or 0))-- + (mainmemory.read_u8(0x1BAC)>combop[2] and 0 or 0)
...
function lateglobals()
	...
	health[2] = mainmemory.read_s16_le(0x1B74)
	combop[1] = mainmemory.read_u8(0x1BA8)
	combop[2] = mainmemory.read_u8(0x1BAC)
...
local function hpdisplay()
	...
	gui.pixelText(  15, 32, math.floor(HP1) .. "/1807", hpcolor, 0x00000000) 
	gui.pixelText( 143, 32, math.floor(HP2) .. "/1807", hpcolor, 0x00000000) 
...
local function magicbox()
	...
	for i = 0,1, 1 do
		...
		local pfacing = bit.band( pface, 64) == 64 and -1 or 1
...
local function projectiles()
	...
	for i = 0,7, 1 do
		...
		local pjattack	= mainmemory.read_u8	(pjstart + 0x200 + 0x80)
		local pjattack2	= mainmemory.read_u8	(pjstart + 0x200 + 0x80 + 0x02)
		local pjenable  = mainmemory.read_s16_le(pjstart - 0x100)
		local facing = bit.band( pjface, 64) == 64 and -1 or 1

		if (pjenable > 0 and pjenable ~= 66 and pjenable ~= 84 and pjenable ~= 88) or (pjenable == 66 and pjattack2 == 0) then
...
local function pausemenu()
	...
	for i = 1,menulength, 1 do
		...
		if option[i] == 1 then
			gui.pixelText( menuoffsetx + 104, menuoffsety + 80 + (i*8), "O", i~=8 and 0xFFFF0000 or 0x00000000, 0x00000000)
		end
...
local function stageselectversus()

	local menulength = 12
	local menuselect = ( mainmemory.read_u8(0x00060E) * 0.5)
	if buttonp1up == 1 then
		menuselect = menuselect + 1
		if menuselect > (menulength - 1) then
			menuselect = 0
		end
	elseif buttonp1down == 1 then
		menuselect = menuselect - 1
		if menuselect < 0 then
			menuselect = menulength - 1
		end
	end

	if buttonp2up == 1 then
		menuselect = menuselect + 1
		if menuselect > menulength then
			menuselect = 1
		end
	elseif buttonp2down == 1 then
		menuselect = menuselect - 1
		if menuselect < 1 then
			menuselect = menulength
		end
	...
	if true then
		gui.pixelText( 70, 8, "Stage Select:", 0xFFFFFFFF, 0x00000000)
		gui.pixelText( 124, 8, stageselectlabels[menuselect], 0xFFFFFFFF, 0x00000000)
	end
	mainmemory.write_u8(0x00060E, (menuselect * 2))
	
	if menuselect == 0 then
		mainmemory.write_u8(0x000038, 10)
	end
end

local function lifebars()

	local HP1 = (mainmemory.read_s16_le(0x1B70) / 16)
	local HP2 = (mainmemory.read_s16_le(0x1B74) / 16)

	gui.drawImage("images/Lifebars.png", 4, 2, 100 * (HP1/1807)) 

end

local function displaydamage()

-- p2 is first
	for i = 0,1, 1 do
		local offset = 0x1B70 + (i * (0x04))
		if firstframeofhitp[2-i] == 1 then --and combop[i+1] < mainmemory.read_u8(offset + 0x38) then --		if mainmemory.read_s16_le(offset) < health[i] then
			recenthit[i] = math.floor((health[i+1] - mainmemory.read_s16_le(offset))) / 16
			if mainmemory.read_u8(0x1BAC - (i * 0x04)) == 1 then --offset + 0x38
				totaldamage[i+1] = 0
			end
			totaldamage[i+1] = totaldamage[i+1] + recenthit[i]
		end
		gui.pixelText(  29 + (i * 140),	 0, "Damage:", 0xFFFFFFFF, 0x00000000)
		gui.pixelText( 	35 + 22 + (i * 140),  0, recenthit[i], 0xFFFF0000, 0x00000000)
	
		gui.pixelText(   5 + (i * 140),	 8, "Total Damage:", 0xFFFFFFFF, 0x00000000)
		gui.pixelText( 	35 + 22 + (i * 140),  8, totaldamage[i+1], 0xFFFF0000, 0x00000000)
		
	end
end
local function volumeadjust()
--	if gamestate == 8 and matchstate == 40 then
--		memory.usememorydomain("APURAM")
--		memory.write_u8(0x0C, 0)
--		memory.usememorydomain("WRAM")
--	else
		memory.usememorydomain("APURAM")
		memory.write_u8(0x0C, (volume * (pause < 0 and .5 or 1)))
		memory.usememorydomain("WRAM")
--	end
end

function test()
		gui.pixelText(  29 , 100, "Damage:", 0xFFFFFFFF, 0x00000000)
end
...
--gui.drawImage("images/Portrait09.png", 0, 0, 84, 155) 
	if gamestate > 0 then
		reset()
	end
	if gamestate == 6 then
		stageselectversus()
	elseif gamestate == 8 and matchstate == 32 then
		gui.pixelText( 140,  8, " ", 0x00000000, 0x00000000)
		if option[9] == -1 then
			displaydamage()
		end
		----------------------------------------
		...
		---------------------------------------- Lifebar Disable
		if option[6] == 1 then
			mainmemory.write_u8(0x10, 4)
		elseif option[6] == -1 then
			mainmemory.write_u8(0x10, 2)
		end
		----------------------------------------
		if option[4] == -1 then
			player()
		...
		if option[7] == 1 then
			mainmemory.write_u8(0x1090, 0)
			mainmemory.write_u8(0x1094, 0)
		end
		settings()
		----------------------------------------
	else
	...
	--- Add state select music if Stage 00 is selected.
--	if mainmemory.read_u8(0x00060E) == 0 and gamestate > 5 and gamestate < 9 and matchstate < 32 then
--		mainmemory.write_u8(0x000038, 10)
--	end
	volumeadjust()
	lateglobals()
...
-- Runs all the main code
event.onframestart( main )

while true do
--	if gamestate == 20 then
--		mainmemory.write_u8(0x0546, 0)
--	end

--	if gamestate == 8 then
--		mainmemory.write_u8(0x08, ((mainmemory.read_u8(0x08)%4 == 0 and 1 or 0) * 2) + mainmemory.read_u8(0x08)  )
--		mainmemory.write_u8(0x0144, (matchstate < 32 or matchstate > 41) and 23 or 19)
--		mainmemory.write_u8(0x0A, ((mainmemory.read_u8(0x0A)%4 == 0 and 1 or 0) * 2) + mainmemory.read_u8(0x0A)  )
--	end
